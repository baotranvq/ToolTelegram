  ...
  async getProxy() {
    try {
      const response = await axios.get(
        "https://wwproxy.com/api/client/proxy/available?key=UK-fda7c0fb-0199-478d-8a87-e46085b78790&provinceId=-1"
      );
      if (response.status === 200) {
        const httpProxy = `http://${response.data.data.proxy}`;

        return httpProxy;
      } else {
        throw new Error(
          `Không thể  lấy proxy. Status code: ${response.status}`
        );
      }
    } catch (error) {
      throw new Error(`Error lấy proxy lỗi: ${error.message}`);
    }
  }
  ...
  async main() {
    ...
    while (true) {
      for (let i = 0; i < queryIds.length; i++) {
        ...
        const wwProxy = await this.getProxy();
        this.proxyAgent = new HttpsProxyAgent(wwProxy);

        let proxyIP;
        try {
          proxyIP = await this.checkProxyIP(wwProxy);
          console.log("current proxy: ", proxyIP);
        } catch (error) {
          this.log(
            `Không thể sử dụng proxy ${this.proxy}: ${error.message}`,
            "error"
          );
          continue;
        }
